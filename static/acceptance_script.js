...
function showCompletionScreen() {
     ...
     if (defectsList.length > 0) {
          // Показываем блок со списком, скрываем сообщение "нет дефектов"
          noDefectsMessage.style.display = 'none';
          summaryList.style.display = 'block'; // Убедимся, что <ul> виден

          defectsList.forEach(defect => {
               const li = document.createElement('li');
               // --- ИЗМЕНЕНИЕ ЗДЕСЬ ---
               // Получаем ID элемента и ищем описание в новом словаре
               const itemId = defect.item || 'unknown';
               const description = DEFECT_DESCRIPTIONS_JS[itemId] || `обнаружен дефект (${itemId})`; // Запасной текст
               // Формируем строку: Комната: Описание дефекта
               li.textContent = `${defect.room}: ${description}`;
               // --- КОНЕЦ ИЗМЕНЕНИЯ ---
               summaryList.appendChild(li);
          });
     } else {
          // Показываем сообщение "нет дефектов", скрываем список
